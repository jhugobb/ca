...
{	
  ...
  // v3 = glm::vec3(0, -3, 3.5);
	...
	for (unsigned int i = 0; i < 1; i++) {
		Particle* part = new Particle(0,-5,0);
		particles.push_back(part);
		
		m_skeleton = new Model();
		m_skeleton->setPath("../skeleton/");

		if(!m_skeleton->onInit("../skeleton.cfg")) {
			delete m_skeleton;
	...
	particle_mesh->buildCube(0.3, false, glm::vec3(0,1,0));
...
bool Scene::init(int argc, char* argv[])
{
	...
	m_skeleton->onUpdate(1);
	// m_skeleton->sendToOpenGL(basicProgram);
...
void Scene::update(int deltaTime, bool forward, bool back, bool left, bool right, bool bPlay) 
{
	...
	// m_skeleton->onUpdate(deltaTime*1000.0f);
