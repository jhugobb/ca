...
void Model::sendToOpenGL(ShaderProgram &program) {
    ...
    for(submeshId = 0; submeshId < submeshCount; submeshId++)
    {
      ...
      if(pCalRenderer->selectMeshSubmesh(meshId, submeshId))
      {
        ...
        for (uint n = 0; n < vertexCount; n++) {
          vertices.push_back(glm::vec3(meshVertices[n][0], meshVertices[n][1], meshVertices[n][2]));
        }
        ...
        for (uint n = 0; n < faceCount; n++) {
          triangles.push_back(meshFaces[n][0]);
          triangles.push_back(meshFaces[n][1]);
          triangles.push_back(meshFaces[n][2]);
        }
